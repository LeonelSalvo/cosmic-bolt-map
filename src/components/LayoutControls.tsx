import { memo } from 'react';

interface Props {
  isSmartLayoutEnabled: boolean;
  onToggleSmartLayout: () => void;
}

const LayoutControls = memo(function LayoutControls({ 
  isSmartLayoutEnabled, 
  onToggleSmartLayout 
}: Props) {
  return (
    <div className="layout-controls">
      <button 
        className={`layout-button ${isSmartLayoutEnabled ? 'active' : ''}`}
        onClick={onToggleSmartLayout}
        title={isSmartLayoutEnabled ? 'Disable Smart Layout' : 'Enable Smart Layout'}
      >
        <svg viewBox="0 0 24 24" width="24" height="24">
          <path 
            fill="currentColor" 
            d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm-5-9h10v2H7z"
          />
        </svg>
      </button>
    </div>
  );
});

export default LayoutControls;